...
func TestProxyPreservesToolsAndForwardsCalls(t *testing.T) {
	...
	if len(tools.Tools) != 1 || tools.Tools[0].Name != "telegram_test" || tools.Tools[0].Annotations == nil || !tools.Tools[0].Annotations.ReadOnlyHint {
		t.Fatalf("unexpected proxied tools: %#v", tools.Tools)
	}
	result, err := clientSession.CallTool(ctx, &mcp.CallToolParams{Name: "telegram_test", Arguments: map[string]any{}})
	if err != nil {
		t.Fatal(err)
	}
