...
func (s *Server) addMediaTools(server *mcp.Server) {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "telegram_download_attachment", Description: "Download the original voice, video note, or image to a private cloud cache. Returns an expiring URL requiring the MCP bearer token. Does not call an AI provider.", Annotations: write}, s.downloadAttachment)
...
func (s *Server) getAttachment(ctx context.Context, _ *mcp.CallToolRequest, in attachmentInput) (*mcp.CallToolResult, media.Attachment, error) {
	a, err := s.media.Metadata(ctx, in.Chat, in.MessageID)
	return nil, a, err
}
func (s *Server) downloadAttachment(ctx context.Context, _ *mcp.CallToolRequest, in attachmentInput) (*mcp.CallToolResult, media.Download, error) {
	d, err := s.media.Download(ctx, in.Chat, in.MessageID)
	if err == nil {
		d.DownloadURL = s.publicURL + "/mcp/media/" + d.FileID
	}
	return nil, d, err
}
func (s *Server) transcribeMedia(ctx context.Context, _ *mcp.CallToolRequest, in transcribeInput) (*mcp.CallToolResult, media.Job, error) {
	j, err := s.media.Start(ctx, in.Chat, in.MessageID, "transcription", media.Options{Model: in.Model, Keywords: in.Keywords, Languages: in.Languages}, in.ConfirmPaid)
	return nil, j, err
}
...
func (s *Server) getTranscription(ctx context.Context, _ *mcp.CallToolRequest, in mediaResultInput) (*mcp.CallToolResult, media.Job, error) {
	j, err := s.media.Get(ctx, in.Chat, in.MessageID, in.JobID)
	if err == nil && j.Operation != "transcription" {
		return nil, media.Job{}, media.Fail("job_operation_mismatch")
	}
	return nil, j, err
}
func (s *Server) getImageAnalysis(ctx context.Context, _ *mcp.CallToolRequest, in mediaResultInput) (*mcp.CallToolResult, media.Job, error) {
	j, err := s.media.Get(ctx, in.Chat, in.MessageID, in.JobID)
...
func (s *Server) downloadFile(w http.ResponseWriter, r *http.Request) {
	...
	file, meta, err := s.media.OpenDownload(r.Context(), strings.TrimPrefix(r.URL.Path, "/mcp/media/"))
	if err != nil {
		http.NotFound(w, r)
		return
	}
