...
- Recognition runs on Fly through OpenRouter. Do not launch a local Telegram session, Telegram Desktop, or a local inference model.
...
- Poll `telegram_get_transcription` or `telegram_get_image_analysis` using the returned job ID and original chat/message IDs. Respect `next_attempt_at`; jobs continue on the server.
